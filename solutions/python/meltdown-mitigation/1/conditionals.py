...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    pc_value = ((voltage * current)/theoretical_max_power)*100
    if pc_value >= 80:
        return 'green'
    elif pc_value >= 60 and pc_value < 80:
        return 'orange'
    elif pc_value >= 30 and pc_value < 60:
        return 'red'
    else:
        return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if range < threshold * 90/100:
        return 'LOW'
    elif range < (threshold + (threshold * 10/100)) and range >= threshold:
        return 'NORMAL'
    elif range <= threshold and range > (threshold - (threshold * 10/100)):
        return 'NORMAL'
    else:
        return 'DANGER'
